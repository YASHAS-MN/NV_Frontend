...
import { inferCategory } from "@/lib/catalog-utils";
...
export function SurfPage() {
  ...
  const entries = useMemo(() => {
    const filtered = Object.entries(marketState).filter(([assetName]) => {
      ...
      const inferredCategory = inferCategory(assetName);
      return matchesQuery && inferredCategory === activeCategory;
    });
  ...
  const handleUpload = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!uploadForm.file || !uploadForm.assetName || !uploadForm.price) return;
    ...
    setUploadForm({ assetName: "", price: "", file: null });
  };
      ...
      <main className="mx-auto w-full max-w-[1880px] px-4 pb-8 pt-4 sm:px-6 lg:px-8">
        <section className="space-y-6">
                ...
                <div className="flex justify-center">
                  <div className="flex w-full max-w-[920px] items-center gap-3 rounded-full border border-white/10 bg-black/20 px-5 py-4">
                    ...
                    <input
                      ...
                      placeholder="Search verified products, code packs, art, tools, and documents"
              ...
              >
                <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                  <div className="max-w-4xl">
                    <SignalPill tone="accent">Surf intro</SignalPill>
                    <h1 className="mt-4 text-3xl font-semibold text-white sm:text-4xl">
                      Upload, discover, and trade verified software in the NEBULAverse marketplace.
                    </h1>
                    <p className="mt-4 text-sm leading-8 text-nebula-muted">
                      This is the main software exchange surface of the platform. Search live listings,
                      upload your own artifact, and move into escrow-backed transactions from one flow.
                    </p>
                  </div>

                  <form onSubmit={handleUpload} className="grid w-full max-w-[620px] gap-3">
                    <div className="grid gap-3 md:grid-cols-[1.2fr_0.55fr_0.75fr]">
                      <input
                        value={uploadForm.assetName}
                        onChange={(event) =>
                          setUploadForm((current) => ({ ...current, assetName: event.target.value }))
                        }
                        placeholder="software-name.py"
                        className="rounded-[20px] border border-white/10 bg-black/20 px-4 py-4 text-sm text-white outline-none transition placeholder:text-white/30 focus:border-nebula-accent/50"
                      />
                      <input
                        type="number"
                        min="0"
                      ...
                      <input
                        ...
                        className="block w-full rounded-[20px] border border-dashed border-white/15 bg-black/20 px-4 py-[14px] text-sm text-white file:mr-3 file:rounded-full file:border-0 file:bg-nebula-accent file:px-4 file:py-2 file:text-sm file:font-medium file:text-slate-950"
                      />
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      >
                        {isUploadBusy ? <LoaderCircle className="size-4 animate-spin" /> : <UploadCloud className="size-4" />}
                        {isUploadBusy ? "Injecting to mempool" : "Upload software"}
                      </button>
                      <span className="text-sm text-nebula-muted">
                        {isRefreshing ? "Refreshing protocol marketplace..." : `${entries.length} results in ${activeCategory}`}
                      </span>
                    </div>
                  </form>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-start">
              <button className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm text-white/76 transition hover:bg-white/10">
                Load more
              </button>
            </div>
          </div>
            ...
            <>
              ...
              >
                ...
                <div className="mt-6 space-y-6">
                  <div>
                    <div className="mb-3 text-xs uppercase tracking-[0.26em] text-white/44">Sort options</div>
                  ...
                  <div>
                    <div className="mb-3 text-xs uppercase tracking-[0.26em] text-white/44">Categories</div>
                    <div className="grid gap-3">
                  ...
                  <div>
                    <div className="mb-3 text-xs uppercase tracking-[0.26em] text-white/44">Trust signals</div>
...
function SurfCard({
  ...
  const { currentWalletId, isLoading } = useNebula();
    ...
    <article className="group relative overflow-hidden rounded-[30px] border border-white/8 bg-[linear-gradient(180deg,rgba(17,11,29,0.88),rgba(8,9,18,0.92))] p-4 transition duration-300 hover:-translate-y-1 hover:border-nebula-accent/22">
      ...
      <div className="mt-6 flex items-center justify-between gap-4">
        ...
        <button 
          ...
          className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-50"
          style={{
